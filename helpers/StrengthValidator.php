...
class StrengthValidator extends Validator
{
    ...
    /**
     *
     * @var string|array preset
     * If this is not null, the preset parameters will override the validator level params
     * This can be specified in one of the following forms:
     * - a string: one of the preset constants as defined in [[self::$_presets]].
     * - an array of elements: set of rules as defined in [[self::$_rules]].
     *
     */
    public $preset;
    ...
    protected function applyPreset()
    {
        ...
        //var_dump($this->preset);exit;
        if (is_array($this->preset)) {
            foreach ($this->preset as $param => $value) {
                if (array_key_exists($param, self::$_rules)) {
                        $this->$param = $value;
                } else {
    ...
    protected function setRuleMessages()
    {
        if ($this->message === null) {
            $this->message = Yii::t('user', '{attribute} must be a string');
        }
        foreach (self::$_rules as $rule => $setup) {
            $param = "{$rule}Error";
            if ($this->$rule !== null) {
    ...
    protected static function getRuleMessage($rule)
    {
        switch ($rule) {
            ...
            case self::RULE_SPACES:
                return Yii::t('user', '{attribute} cannot contain any spaces');
            case self::RULE_USER:
                return Yii::t('user', '{attribute} cannot contain the username');
            case self::RULE_EMAIL:
                return Yii::t('user', '{attribute} cannot contain an email address');
            case self::RULE_LOW:
                return Yii::t(
                    'user',
